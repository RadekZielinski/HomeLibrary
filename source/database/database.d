...
class Database {
	...
	this(string dbAddr) {
		this.url = join(["sqlite:", dbAddr]);
		try {
			Connection connection = createConnection(this.url);
			scope(exit) connection.close();
			Statement stmnt = connection.createStatement();
			scope(exit) stmnt.close();
			...
			stmnt.executeUpdate("
				...
				(id integer primary key autoincrement not null, 
				title varchar(200) not null, author_id int not null, 
			...
			stmnt.executeUpdate("
				create table if not exists albums (id integer primary key autoincrement not null, 
			...
			stmnt.executeUpdate("
				...
				(id integer primary key autoincrement not null, 
				borrower_id int not null, book_uuid varchar not null, 
			...
			stmnt.executeUpdate("insert into carrier_types(name) values ('Kaseta');");
		}
		catch(Exception e) {
			writeln(e.msg);
		}
